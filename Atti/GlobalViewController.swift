    ...
    override func viewDidLoad() {
        ...
        ref.observe(.value) { snapshot in
            ...
            do {
                ...
                let diaryData = try JSONDecoder().decode([String: Diary].self, from: jsonData)
                let diaryListbeforeSort = Array(diaryData.values)
                
                // -> 정렬 순서 나중에 바꾸기
                self.diaryList = diaryListbeforeSort.sorted {  $0.order > $1.order }
